  ...
  return (
    <li className="aspect-square w-full overflow-hidden group cursor-pointer relative bg-gray-900">
      {!work.image || imageError ? (
        // Placeholder - mantiene proporción cuadrada perfecta
        <div className="absolute inset-0 flex justify-center items-center bg-gradient-to-br from-gray-800 to-gray-900">
          <div className="text-center px-6">
            ...
            {imageError && <p className="text-gray-400 text-xs sm:text-sm mt-2">Error al cargar imagen</p>}
          </div>
        </div>
      ) : (
        <>
      ...
      {/* Accesibilidad */}
      <span className="sr-only">{work.image ? `Trabajo: ${work.alt}` : "Galería de trabajos recientes"}</span>
    </li>
  )
}
